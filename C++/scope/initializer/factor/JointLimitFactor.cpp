...
#include <scope/initializer/factor/JointLimitFactor.h>
...
JointLimitFactor::JointLimitFactor(int joint, const AlignedVector<Matrix6> &W,
                                   ...
                                   int index)
    : Factor(name, index), mJoint(joint), mnLayers(W.size()),
...
int JointLimitFactor::linearize(const Matrix3 &Omega, const Evaluation &eval,
  ...
  math::skew3::multR<math::OPS::SUB>(
      Omega.col(0), lin.derivatives[0].leftCols<3>(), lin.jacobian);
  math::skew3::multR<math::OPS::SUB>(
      Omega.col(1), lin.derivatives[0].rightCols<3>(), lin.jacobian);
