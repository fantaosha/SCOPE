...
namespace scope {
FullVertexPinholeCameraFactor::FullVertexPinholeCameraFactor(
    ...
    : VertexPinholeCameraFactor(pose, vParams, vDirs, v, sigma, eps,
                                measurement, confidence, name, index, active) {
  mvParams.push_back(camParam);
}

int FullVertexPinholeCameraFactor::evaluate(
...
int FullVertexPinholeCameraFactor::linearize(
    const AlignedVector<Pose> &poses, const AlignedVector<VectorX> &shapes,
    const AlignedVector<Matrix3> &joints, const AlignedVector<VectorX> &params,
...
int FullVertexPinholeCameraFactor::evaluate(const Pose &pose,
  ...
  VertexPinholeCameraFactor::evaluate(pose, vertexParam, eval.camMeasurement,
                                      eval);

  return 0;
}

int FullVertexPinholeCameraFactor::linearize(const Pose &pose,
                                             const VectorX &vertexParam,
                                             const VectorX &camParam,
                                             const Vector2 &measurement,
  ...
  VertexPinholeCameraFactor::linearize(pose, vertexParam, eval.camMeasurement,
                                       eval, lin);
